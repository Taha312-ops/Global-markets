      ...
      >
        ...
        <g clipPath="url(#mapClip)">
          ...
          {visibleCountries.map(c => {
            ...
            return (
              <g key={c.code} onClick={(e) => { e.stopPropagation(); onCountrySelect(c.code); }}
                onMouseEnter={() => setHovered(c.code)}
                onMouseLeave={() => setHovered(null)}
                style={{ cursor: "pointer" }}
              >
                {(isSelected || isHovered) && (
                  <circle cx={c.tx} cy={c.cy} r="18" fill="#00d4ff" opacity="0.12" />
                )}
                <circle cx={c.tx} cy={c.cy} r={isSelected ? 9 : isHovered ? 7 : 5}
                  fill={isSelected ? "#00d4ff" : isHovered ? "#4de8ff" : "#1e90ff"}
                  stroke={isSelected ? "#fff" : "#00d4ff"}
                ...
                {isSelected && (
                  <>
                    <circle cx={c.tx} cy={c.cy} r="14" fill="none" stroke="#00d4ff" strokeWidth="1" opacity="0.5" strokeDasharray="3 3">
                      <animateTransform attributeName="transform" type="rotate" from={`0 ${c.tx} ${c.cy}`} to={`360 ${c.tx} ${c.cy}`} dur="4s" repeatCount="indefinite" />
                    </circle>
                  </>
                )}
                {(isHovered || isSelected) && (
                  <text x={c.tx} y={c.cy - 15} textAnchor="middle" fill="#e0f4ff" fontSize="10" fontFamily="'Courier New', monospace" fontWeight="bold">
                    {c.name}
                  </text>
                )}
              </g>
        ...
        {/* Vignette overlay */}
        <rect x="0" y="0" width="60" height={GLOBE_H} fill="url(#fadeLeft)" rx="12" />
        <defs>
          <linearGradient id="fadeLeft" x1="0%" x2="100%">
            <stop offset="0%" stopColor="#060e1a" />
            <stop offset="100%" stopColor="transparent" />
          </linearGradient>
          <linearGradient id="fadeRight" x1="0%" x2="100%">
            <stop offset="0%" stopColor="transparent" />
            <stop offset="100%" stopColor="#060e1a" />
          </linearGradient>
        </defs>
        <rect x={GLOBE_W - 60} y="0" width="60" height={GLOBE_H} fill="url(#fadeRight)" />

        {/* Drag hint */}
        <text x={GLOBE_W / 2} y={GLOBE_H - 12} textAnchor="middle" fill="#1e5a8a" fontSize="10" fontFamily="monospace">
          ← DRAG TO ROTATE · CLICK COUNTRY TO SELECT →
        </text>
      </svg>
    </div>
  );
...
function NewsPanel({ title, query, icon }) {
  ...
  useEffect(() => {
    ...
    fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: "user",
          content: `Search for the latest ${query}. Return a JSON array (no markdown, no backticks, just raw JSON) of exactly 5 articles, each with: title (string), summary (string, 1-2 sentences), source (string), sentiment (one of: positive, negative, neutral). Only return the JSON array.`
        }]
      })
    })
      .then(r => r.json())
      ...
      .catch(e => { setError("Failed to load news."); setLoading(false); });
  }, [query]);

  const sentimentColor = s => s === "positive" ? "#00e676" : s === "negative" ? "#ff5252" : "#90a4ae";
  const sentimentLabel = s => s === "positive" ? "▲" : s === "negative" ? "▼" : "●";

  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "14px 18px 10px", borderBottom: "1px solid #1a3a5c", display: "flex", alignItems: "center", gap: 8 }}>
        <span style={{ fontSize: 16 }}>{icon}</span>
        <span style={{ fontSize: 12, fontFamily: "monospace", color: "#00d4ff", fontWeight: "bold", letterSpacing: 2 }}>{title}</span>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 14px" }}>
        {loading && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, paddingTop: 10 }}>
            {[1,2,3,4,5].map(i => (
              <div key={i} style={{ background: "#0d1f35", borderRadius: 8, padding: 12, animation: "pulse 1.5s infinite" }}>
                <div style={{ height: 10, background: "#1a3a5c", borderRadius: 4, marginBottom: 8, width: "80%" }} />
                <div style={{ height: 8, background: "#1a3a5c", borderRadius: 4, width: "60%" }} />
              </div>
            ))}
          </div>
        )}
        {error && <div style={{ color: "#ff5252", fontSize: 12, fontFamily: "monospace", paddingTop: 10 }}>{error}</div>}
        {articles.map((a, i) => (
          <div key={i} style={{
            background: "linear-gradient(135deg, #0d1f35 0%, #0a1828 100%)",
            border: "1px solid #1a3a5c",
            borderLeft: `3px solid ${sentimentColor(a.sentiment)}`,
            borderRadius: 8,
            padding: "12px 14px",
            marginBottom: 10,
            ...
            onMouseEnter={e => { e.currentTarget.style.borderColor = "#00d4ff"; e.currentTarget.style.transform = "translateX(2px)"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = "#1a3a5c"; e.currentTarget.style.borderLeftColor = sentimentColor(a.sentiment); e.currentTarget.style.transform = ""; }}
          >
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
              <div style={{ fontSize: 11, fontFamily: "'Courier New', monospace", color: "#00d4ff", fontWeight: "bold", lineHeight: 1.4, flex: 1 }}>{a.title}</div>
              <span style={{ color: sentimentColor(a.sentiment), fontSize: 13, marginLeft: 8, flexShrink: 0 }}>{sentimentLabel(a.sentiment)}</span>
            </div>
            <div style={{ fontSize: 10, color: "#6a9bbf", lineHeight: 1.5, fontFamily: "Georgia, serif" }}>{a.summary}</div>
